...
.card:hover { transform:translateY(-2px); box-shadow:0 4px 14px rgba(16,24,40,.08), 0 10px 26px rgba(16,24,40,.06);
  border-color:var(--bd); }
.card.acc::before, .card.red::before, .card.green::before, .card.amber::before { opacity:.9; }
...
.sec-h h2 { margin:0; font-size:16.5px; font-weight:700; display:flex; align-items:center; gap:8px; letter-spacing:.2px; }
  .sec-h h2::before { content:""; width:4px; height:15px; border-radius:2px; background:linear-gradient(var(--acc),var(--acc-strong)); flex:none; }
.sec-h .muted { color:var(--mut); font-size:12.5px; }
.sec-h .sp { flex:1; }
...
.pt:hover { background:var(--hover); box-shadow:var(--shadow-sm); border-color:var(--bd); }
.pt .row1 { display:flex; align-items:center; gap:8px; flex-wrap:wrap; }
.pt .zh { font-weight:650; font-size:14px; }
...
.lb h2 { font-size:16.5px; margin:26px 0 10px; padding:2px 0 8px 10px; border-left:3px solid var(--acc); border-bottom:1px solid var(--bd); }
...
.lb p { margin:12px 0; line-height:1.9; }
.lb th, .lb td { border-bottom:1px solid var(--bd); padding:6px 9px; text-align:left; }
  .lb tbody tr:nth-child(even) { background:var(--bd-soft); }
...
<div class="topbar">
  <div class="topbar-in">
    <h1>俄乌战争<span class="dot">·</span>每日情报</h1>
...
<div class="wrap">
  ...
  <section class="sec" id="events">
    <div class="sec-h">
      ...
      <span class="sp"></span>
      <a class="btn" id="eventsToggle" href="javascript:void(0)" onclick="toggleEvents()">展开检索 ▾</a>
    </div>
    <div class="sec-b" id="eventsBody" hidden>
      <div class="filters">
...
function toggleEvents(){ var b=document.getElementById('eventsBody'), t=document.getElementById('eventsToggle'); if(!b||!t) return; var o=b.hidden; b.hidden=!o; t.textContent=o?'收起检索 ▴':'展开检索 ▾'; }
...
def main() -> None:
    ...
    stats = (
        f'<div class="card acc"><div class="num">{len(rows)}</div><div class="lbl">事件总数</div></div>'
        f'<div class="card red"><div class="num">{today_events}</div><div class="lbl">今日事件（{latest_date}）</div></div>'
        f'<div class="card amber"><div class="num">{disagreements}</div><div class="lbl">分歧事件</div></div>'
        f'<div class="card green"><div class="num">{len(counter_type)}</div><div class="lbl">维度覆盖</div></div>'
        f'<div class="card"><div class="num">{len(counter_theater)}</div><div class="lbl">战区覆盖</div></div>'
        f'<div class="card"><div class="num">{len(items)}</div><div class="lbl">简报份数</div></div>'
    )
