<!--
周复盘 · 深度解析写作提纲 v2（给维护者 / 接手 AI 用）
本文件不被流水线读取，是"如何写周度深度解析"的指南。
...
## 输入 / Inputs
- 自动数据层：`briefings/weekly/{YYYY}-W{WW}.md`（各维度分组 + 信源对照表）
- 原始数据：`data/master/events.csv` 切片 `{date_start} ~ {date_end}`
